"""Self-distillation baseline: RGB ResNet34 teacher -> RGB ResNet34 student.

Both teacher and student see the same strongly-augmented RGB input
("Consistent Teaching"), so a single DataLoader suffices. Use this script to
reproduce the ``Self Distilled`` row in the paper's tables.
"""
...
# ---------------------------------------------------------------------------
# DataModule (single loader; teacher and student share the input)
# ---------------------------------------------------------------------------

class SelfKDDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.train_ds = RGBFromTSVDictDataset(rgb_paths, self.class_to_idx, tfm_rgb_strong())
...
# ---------------------------------------------------------------------------
# LightningModule: both networks receive the same strong-aug input
# ---------------------------------------------------------------------------

class LitKDSelf(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_s = self(x_rgb)
        with torch.no_grad():
            logits_t = self.teacher(x_rgb)

        loss_ce = self.ce_loss(logits_s, y)
        ...
        self.log_dict({
            ...
            'train/acc_ce': (logits_s.argmax(1) == y).float().mean(),
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Self-distillation (RGB teacher -> RGB student)')
    ...
    p.add_argument('--teacher_ckpt', required=True,
                   help='Lightning checkpoint of the RGB ResNet34 teacher.')
    ...
    p.add_argument('--run_name', default='kd_resnet34_self')
