"""Gray-distillation baseline: grayscale ResNet34 teacher -> RGB ResNet34 student.

The teacher receives a weakly-augmented 3-channel grayscale view while the
student receives a weakly-augmented RGB view; a parallel CE branch on a
strongly-augmented RGB view regularises the student. Reproduces the
``Gray Distilled`` row of the paper's ablation.
"""
...
class GrayKDDataModule(L.LightningDataModule):
    """CE branch: strong-aug RGB.  KD branch: weak-aug RGB + weak-aug gray3."""
    ...
    def setup(self, stage=None):
        ...
        self.train_ds_ce = RGBFromTSVDictDataset(rgb_paths, self.class_to_idx, tfm_rgb_strong())
        self.train_ds_kd = RgbAndGrayDataset(
            rgb_paths, self.class_to_idx,
            rgb_tfm=tfm_rgb_weak(), gray_tfm=tfm_gray3_weak(),
        )
        self.val_ds = datasets.ImageFolder(self.rgb_val_root, transform=tfm_rgb_weak())
        self._sampler = SyncedDistributedSampler(self.train_ds_ce, seed=self.seed, shuffle=True, drop_last=True)

    def _loader(self, ds: Dataset, *, drop_last: bool) -> DataLoader:
        return DataLoader(
            ds, batch_size=self.batch_size, sampler=self._sampler if drop_last else None,
            shuffle=False, num_workers=self.num_workers, pin_memory=True,
            drop_last=drop_last, persistent_workers=(self.num_workers > 0),
        )

    def train_dataloader(self):
        return {
            'ce': self._loader(self.train_ds_ce, drop_last=True),
            'kd': self._loader(self.train_ds_kd, drop_last=True),
        }
...
class LitKDGrayTeacher(L.LightningModule):
    ...
    def training_step(self, batch_dict, batch_idx):
        # CE branch (strong-aug RGB only).
        ce = batch_dict['ce']
        y_ce = ce['label'].long()
        logits_ce = self(ce['rgb'])
        loss_ce = self.ce_loss(logits_ce, y_ce)
        acc_ce = (logits_ce.argmax(1) == y_ce).float().mean()

        # KD branch (weak-aug RGB -> student, weak-aug gray3 -> teacher).
        kd = batch_dict['kd']
        y_kd = kd['label'].long()
        logits_s = self(kd['rgb'])
        with torch.no_grad():
            logits_t = self.teacher(kd['gray'])
        loss_kd = kd_kl_loss(logits_s, logits_t, self.hparams.temperature)
        acc_kd = (logits_s.argmax(1) == y_kd).float().mean()
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Gray-distillation (Gray teacher -> RGB student)')
    ...
    p.add_argument('--teacher_ckpt', required=True,
                   help='Lightning checkpoint of the 3-channel grayscale ResNet34 teacher.')
    ...
    p.add_argument('--run_name', default='kd_resnet34_grayteacher')
    p.add_argument('--precision', default='32-true')
    p.add_argument('--devices', type=int, nargs='+', default=[0, 1, 2])
