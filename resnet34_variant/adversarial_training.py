"""Adversarial training (PGD) baseline used for the "Adversarial Training" row in the paper.

Fine-tunes a ResNet34 (optionally initialised from a pretrained checkpoint)
against on-the-fly PGD-generated adversarial examples on ImageNet-1k.
"""
...
def tfm_train_weak() -> T.Compose:
    """Light augmentation for adversarial training (strong aug would interfere with PGD noise)."""
    return T.Compose([
...
class LitResNet34Adv(L.LightningModule):
    def __init__(self, lr: float, weight_decay: float, eps: float, alpha: float,
                 iters: int, ckpt_path: Optional[str] = None):
        super().__init__()
    ...
    def pgd_attack(self, images, labels):
        ...
        adv_images = images + torch.empty_like(images).uniform_(
            -self.hparams.eps, self.hparams.eps,
        )

        for _ in range(self.hparams.iters):
            adv_images.requires_grad = True
            loss = self.ce(self.net(adv_images), labels)
            grad = torch.autograd.grad(loss, adv_images)[0]
            adv_images = adv_images.detach() + self.hparams.alpha * grad.sign()
            delta = torch.clamp(adv_images - images, -self.hparams.eps, self.hparams.eps)
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        self.net.eval()
        adv_x = self.pgd_attack(x, y)
        self.net.train()

        logits = self(adv_x)
        loss = self.ce(logits, y)
        acc1 = (logits.argmax(1) == y).float().mean()
        self.log('train/adv_loss', loss, on_step=True, on_epoch=True, prog_bar=True)
        self.log('train/adv_acc1', acc1, on_step=False, on_epoch=True, prog_bar=True)
        return loss
...
class RGBDataModuleFromTSV(L.LightningDataModule):
    def __init__(self, rgb_train_root, rgb_val_root, paired_tsv_path, ratio, seed, batch_size, num_workers):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        self.train_ds = RGBFromTSVDataset(rgb_paths, class_to_idx, tfm_train_weak())
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Adversarial training baseline (PGD)')
    ...
    p.add_argument('--pretrained_ckpt', default=None,
                   help='Optional checkpoint to warm-start from (set to empty to train from scratch).')
    # PGD hyperparameters
    p.add_argument('--adv_eps', type=float, default=8 / 255)
    p.add_argument('--adv_alpha', type=float, default=2 / 255)
    p.add_argument('--adv_iters', type=int, default=10)
    # Training
    ...
    p.add_argument('--run_name', default='resnet34_adv_training')
...
def main():
    ...
    model = LitResNet34Adv(
        lr=args.lr,
        weight_decay=args.weight_decay,
        eps=args.adv_eps,
        alpha=args.adv_alpha,
        iters=args.adv_iters,
