...
def reshape_then_acc_count(event_tensor, augment=None, **kwargs):
    ...
    pos[:, 0] = torch.clamp(pos[:, 0], 0, W - 1)
    pos[:, 1] = torch.clamp(pos[:, 1], 0, H - 1)
    neg[:, 0] = torch.clamp(neg[:, 0], 0, W - 1)
    neg[:, 1] = torch.clamp(neg[:, 1], 0, H - 1)
...
def reshape_then_acc_adj_sort(event_tensor, augment=None, **kwargs):
    ...
    # Temporal discount (epsilon in the denominator avoids division-by-zero)
    patch_size = 5
...
LOADER_REGISTRY = {
    None: 'reshape_then_acc',
    'event_image': 'reshape_then_acc',
    'reshape_then_acc': 'reshape_then_acc',
    'reshape_then_acc_time': 'reshape_then_acc_time',
    'reshape_then_acc_count': 'reshape_then_acc_count',
    'reshape_then_acc_all': 'reshape_then_acc_all',
    'reshape_then_flat_pol': 'reshape_then_flat_pol',
    'binary_event_image': 'reshape_then_flat',
    'reshape_then_flat': 'reshape_then_flat',
    'timestamp_image': 'reshape_then_acc_time_pol',
    'reshape_then_acc_time_pol': 'reshape_then_acc_time_pol',
    'event_histogram': 'reshape_then_acc_count_pol',
    'reshape_then_acc_count_pol': 'reshape_then_acc_count_pol',
    'reshape_then_acc_exp': 'reshape_then_acc_exp',
    'sorted_time_surface': 'reshape_then_acc_sort',
    'reshape_then_acc_sort': 'reshape_then_acc_sort',
    'reshape_then_acc_intensity': 'reshape_then_acc_intensity',
    'dist': 'reshape_then_acc_adj_sort',
    'DiST': 'reshape_then_acc_adj_sort',
    'reshape_then_acc_adj_sort': 'reshape_then_acc_adj_sort',
}
...
class ImageNetDataset(Dataset):
    def __init__(self, cfg, mode='train'):
        ...
        with open(cfg.train_file, 'r') as f:
            self.train_file = [Path(s.strip()) for s in f.readlines()]
        with open(cfg.val_file, 'r') as f:
            self.val_file = [Path(s.strip()) for s in f.readlines()]

        self.map_file = self.train_file if mode == 'train' else self.val_file

        with open(cfg.label_map, 'r') as f:
            labels = [s.split()[1].strip() for s in f.readlines()]
        self.labels = sorted(labels[:1000])

        if getattr(cfg, 'trim_class_count', None) is not None:
            self.labels = self.labels[:cfg.trim_class_count]
            self.map_file = [s for s in self.map_file if s.parent.stem in self.labels]
        ...
        # Event path -> (N, 4) event tensor
        self.event_parser = self.augment_parser(parse_event)

        # (N, 4) event tensor -> network input
        loader_name = LOADER_REGISTRY.get(self.loader_type)
        if loader_name is None:
            raise ValueError(f'Unknown loader_type: {self.loader_type}')
        self.loader = globals()[loader_name]
